...
use App\Models\User;
use App\Exports\UserCatResultExport;
...
class StudentResultController extends Controller
{
    ...
    {
        $record = UserCatResult::findOrFail($id);

        $result = json_decode($record->data);

        return view('backend.result.show', compact('record', 'result'));
    }

    public function exportStudent($id)
    {
        $result = UserCatResult::with('user')->findOrFail($id);
        $result->decoded_data = json_decode($result->data);

        return Excel::download(new UserCatResultExport(collect([$result])), 'student-result-' . $id . '.xlsx');
    }
}
